...
productsRouter.post("/", async (req, res) => {
    try{
        ...
        const productAdded = await productsService.addProduct({title, description, code, price, stock, status, category, thumbnails});
...
productsRouter.put("/:pid", async (req, res) => {
    try {
        ...
        await productsService.updateProduct(parseInt(pid), {title, description, code, price, stock, status, category, thumbnails});
        ...
        res.status(500).send({status: "error", message: "Error del servidor. No se pudo actualizar el producto"});
    }
});    
...
productsRouter.delete("/:pid", async (req, res) => {
    try {
        ...
        res.status(500).send({status: "error", message: "Error del servidor. No se pudo borrar el producto."})
    }
